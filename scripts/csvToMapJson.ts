 ...
 * 
 * Usage: npx ts-node scripts/csvToMapJson.ts <csv_file> <output_json>
 * 
 * Example:
...
function convertCsvToMapJson(
    csvPath: string,
    mapId: string,
    mapName: string,
    options: {
        type?: 'WORLD' | 'TOWN' | 'DUNGEON';
        bgm?: string;
        startX?: number;
        startY?: number;
    } = {}
...
function main() {
    ...
    if (args.length < 2) {
        console.log('Usage: npx ts-node scripts/csvToMapJson.ts <csv_file> <output_json> [mapId] [mapName]');
        console.log('');
        console.log('Examples:');
        console.log('  npx ts-node scripts/csvToMapJson.ts src/data/world/maps/world_01.csv src/data/world/world_01.json world_01 "テストマップ"');
        console.log('  npx ts-node scripts/csvToMapJson.ts src/data/world/maps/town_01.csv src/data/world/town_01.json town_01 "始まりの町"');
        process.exit(1);
    }
